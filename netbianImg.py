...
BASE_URL = "https://pic.netbian.com/"  #  网站地址
...
class Procuder(object):
    ...
    def get_page(self, soup):
        # 获取分页
        alinkLs = soup.find('div', class_='page').find_all('a')
        link_ls = {}
    ...
    def get_image_url(self, soup):
        ...
        for index in range(len(imgls)):
            ...
            # 新增
            img_insert_data = []
            img_insert_data.append(img_soup.find('h1').text.split(" ")[0].encode('gb2312'))
            img_insert_data.append(img_soup.find('h1').text.split(" ")[0].encode('gb2312'))
            ...
            img_insert_data.append(img_soup.find('div', class_='infor').find('a').text.encode('gb2312'))
